"""
Utilitários para a Aula 03 — Estratégias de Mitigação de Drift.

Funções auxiliares para persistência de modelos e métricas, logging
e configuração. Suportam o fluxo completo de experimentação:
geração → treinamento → avaliação → serialização.
...
def setup_logging(
    ...
    logger = logging.getLogger("aula03_mitigacao_drift")
...
def save_model(model: Any, filepath: str | Path) -> Path:
    ...
    Conforme discutido na seção 'Saiba Mais', a serialização é essencial
    para estratégias champion/challenger e shadow mode.

    Args:
        model: Objeto do modelo (BatchChurnModel, OnlineChurnModel, etc.).
...
def load_config(filepath: str | Path | None = None) -> dict[str, Any]:
    ...
    Args:
        filepath: Caminho do arquivo de configuração. Se None,
